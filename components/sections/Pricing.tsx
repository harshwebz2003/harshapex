...
export default function Pricing() {
  ...
  useEffect(() => {
    ...
    intervalId = setInterval(autoScrollNext, 3800);
    ...
    <section id="pricing" ref={sectionRef} className="py-12 sm:py-20 md:py-32 bg-transparent font-body w-full overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="pricing-header text-center mb-8 sm:mb-14">
          ...
          <p className="max-w-xl mx-auto text-[#E7D8FF]/60 text-base sm:text-lg font-light leading-relaxed px-2">
        ...
        <div ref={scrollRef} className="flex md:grid flex-row md:grid-cols-3 overflow-x-auto md:overflow-visible gap-4 sm:gap-8 items-stretch snap-x snap-mandatory scrollbar-none pb-4 md:pb-0 w-full">
          {plans.map((plan) => (
            ...
            >
              {/* Popular badge */}
              {plan.popular && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-[#6DD5C4] to-[#B8C0FF] text-[#0D0B1A] text-[9px] sm:text-[10px] font-extrabold px-3.5 py-1 rounded-full uppercase tracking-wider shadow-md font-mono">
                  Most Popular
                </div>
              )}
              ...
              <div>
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="mt-2.5 sm:mt-3 flex items-baseline gap-2">
                    <span
                      className="text-3xl sm:text-4xl md:text-5xl font-bold text-white font-display tracking-tight"
                    >
                      {plan.price}
                    </span>
                    <span className="text-xs text-[#E7D8FF]/40 font-mono">/{plan.period}</span>
                  </div>
                  <p className="text-xs text-[#E7D8FF]/60 mt-2.5 sm:mt-3 leading-relaxed font-light">{plan.desc}</p>
                </div>
                ...
                <ul className="space-y-2.5 sm:space-y-3 mb-6 sm:mb-8">
                  {plan.features.map((f) => (
                    <li key={f} className="flex items-center gap-2.5 sm:gap-3 text-xs sm:text-sm text-[#E7D8FF]/80">
                      <span className="text-[#6DD5C4] text-xs">✓</span>
                      <span className="font-light">{f}</span>
                    </li>
                  ))}
                  {plan.notIncluded.map((f) => (
                    <li key={f} className="flex items-center gap-2.5 sm:gap-3 text-xs sm:text-sm text-[#E7D8FF]/30">
              ...
              <button
                ...
                className={`w-full py-3 sm:py-3.5 rounded-full text-xs uppercase tracking-[0.08em] font-semibold transition-all duration-300 cursor-pointer font-mono ${
                  plan.popular
        ...
        <div className="mt-10 sm:mt-14 p-6 sm:p-8 rounded-3xl border border-[#6DD5C4]/30 bg-gradient-to-r from-[#1A1630]/80 via-[#120F26]/90 to-[#0D0B1A] flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left shadow-lg">
          <div>
            <div className="text-[11px] sm:text-xs uppercase font-mono tracking-widest text-[#6DD5C4] font-semibold mb-2">Custom Software & POS Development</div>
            <h4 className="text-lg sm:text-xl md:text-2xl font-bold text-white font-display">Need a Specialized POS System, Mobile App, or ERP?</h4>
          ...
          <button
            onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
            className="w-full sm:w-auto shrink-0 px-7 sm:px-8 py-3 sm:py-3.5 rounded-full bg-gradient-to-r from-[#6DD5C4] via-[#B8C0FF] to-[#E7D8FF] text-[#0D0B1A] text-xs font-semibold uppercase tracking-wider font-mono shadow-[0_0_25px_rgba(109,213,196,0.3)] hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer text-center"
          >
